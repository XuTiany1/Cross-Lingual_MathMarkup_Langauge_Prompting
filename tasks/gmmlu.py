...
from prompts.GMMLU.gmmlu_prompt import *
import csv



class gmmlu_task(Task):

    def __init__(self, args):
        super().__init__()

        gmmlu_data_card = "CohereForAI/Global-MMLU-Lite"
        gmmlu_data_dir = "data/global_mmlu_lite/"
        os.makedirs(gmmlu_data_dir, exist_ok=True)
        gmmlu_lang = ['ar', 'bn', 'de', 'en', 'es', 'fr', 'hi', 'id', 'ja' ,'ko', 'pt', 'sw', 'yo', 'zh']

        # Download the dataset in JSON Lines format
        self.download_dataset(gmmlu_data_card, gmmlu_data_dir, gmmlu_lang)
